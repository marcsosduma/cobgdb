...
const char COBOL_RESERVED_WORDS[] = " ABSENT ACCEPT ACCESS ACTION ACTIVATING ACTIVE-CLASS ACTIVE-X ACTUAL ADD ADDRESS ADJUSTABLE-COLUMNS ADVANCING AFTER ALIGNED ALIGNMENT ALL ALLOCATE ALLOWING ALPHABET ALPHABETIC ALPHABETIC-LOWER ALPHABETIC-UPPER ALPHANUMERIC ALPHANUMERIC-EDITED ALSO ALTER ALTERNATE AND ANUM ANY ANYCASE APPLY ARE AREA AREAS ARGUMENT-NUMBER ARGUMENT-VALUE ARITHMETIC AS ASCENDING ASCII ASSIGN AT ATTRIBUTE ATTRIBUTES  AUTO AUTO-DECIMAL AUTO-SKIP AUTO-SPIN AUTOMATIC AUTOTERMINATE AWAY-FROM-ZERO B-AND B-NOT B-OR B-SHIFT-L B-SHIFT-LC B-SHIFT-R B-SHIFT-RC B-XOR BACKGROUND-COLOR BACKGROUND-COLOUR BACKGROUND-HIGH BACKGROUND-LOW BACKGROUND-STANDARD BACKWARD BAR BASED BEEP BEFORE BELL BINARY BINARY-C-LONG BINARY-CHAR BINARY-DOUBLE BINARY-INT BINARY-LONG BINARY-LONG-LONG BINARY-SEQUENTIAL BINARY-SHORT BIT BITMAP BITMAP-END BITMAP-HANDLE BITMAP-NUMBER BITMAP-START BITMAP-TIMER BITMAP-TRAILING BITMAP-TRANSPARENT-COLOR BITMAP-WIDTH BLANK BLINK BLOCK BOOLEAN BOTTOM BOX BOXED BULK-ADDITION BUSY BUTTONS BY BYTE BYTE-LENGTH BYTES C CALENDAR-FONT CANCEL CANCEL-BUTTON CAPACITY CARD-PUNCH CARD-READER CASSETTE CCOL CD CELL CELL-COLOR CELL-DATA CELL-FONT CELL-PROTECTION CELLS CENTER CENTERED CENTERED-HEADINGS CENTURY-DATE CF CH CHAIN CHAINING CHANGED CHARACTER CHARACTERS CHECK-BOX CLASS CLASS-ID CLASSIFICATION CLEAR-SELECTION CLINE CLINES CLOSE COBOL CODE CODE-SET COL COLLATING COLOR COLORS COLOURS COLS COLUMN COLUMN-COLOR COLUMN-DIVIDERS COLUMN-FONT COLUMN-HEADINGS COLUMN-PROTECTION COLUMNS COMBO-BOX COMMA COMMAND-LINE COMMIT COMMON "
                                    ...
                                    " SOURCES SPACE SPACE-FILL SPACES SPECIAL-NAMES SPINNER SQUARE STACK STANDARD STANDARD-1 STANDARD-2 STANDARD-BINARY STANDARD-DECIMAL START START-X START-Y STATEMENT STATIC STATIC-LIST STATUS STATUS-BAR STATUS-TEXT STDCALL STEP STOP STRING STRONG STYLE SUB-QUEUE-1 SUB-QUEUE-2 SUB-QUEUE-3 SUBTRACT SUBWINDOW SUM SUPER SUPPRESS SYMBOL SYMBOLIC SYNC SYNCHRONISED SYNCHRONIZED SYSTEM-DEFAULT SYSTEM-INFO SYSTEM-OFFSET TAB TAB-TO-ADD TAB-TO-DELETE TABLE TALLYING TAPE TEMPORARY TERMINAL-INFO TERMINATE TERMINATION-VALUE TEST TEXT THAN THREAD THREADS THROUGH THRU THUMB-POSITION TILED-HEADINGS TIME TIME-OUT TIMEOUT TIMES TITLE TITLE-POSITION TO TOP TOP-LEVEL TOWARD-GREATER TOWARD-LESSER TRACK TRACK-AREA TRACK-LIMIT TRACKS TRADITIONAL-FONT TRAILING TRAILING-SHIFT TRAILING-SIGN TRANSFORM TRANSPARENT TREE-VIEW TRUE TRUNCATION TYPE TYPEDEF U UCS-4 UNBOUNDED UNDERLINE UNFRAMED UNIT UNIVERSAL UNLOCK UNSIGNED UNSIGNED-INT UNSIGNED-LONG UNSIGNED-SHORT UNSORTED UNSTRING UNTIL UP UPDATE UPDATERS UPON UPPER USAGE USE USE-ALT USE-RETURN USE-TAB USER USER-DEFAULT USING UTF-16 UTF-8 V VAL-STATUS VALID VALIDATE VALIDATE-STATUS VALIDATING VALUE VALUE-FORMAT VALUES VARIABLE VARIANT VARYING VERTICAL VERY-HEAVY VIRTUAL-WIDTH VOLATILE VPADDING VSCROLL VSCROLL-BAR VSCROLL-POS VTOP WAIT WEB-BROWSER WHEN WIDTH WIDTH-IN-CELLS WINDOW WITH WORDS WORKING-STORAGE WRAP WRITE WRITE-ONLY WRITE-VERIFY WRITERS X XML XML-DECLARATION XML-SCHEMA XOR Y YYYYDDD YYYYMMDD ZERO ZERO-FILL ZEROES ZEROS 'ADDRESS OF' COB-CRT-STATUS DEBUG-ITEM "
...
const char COBOL_SPECIAL_WORDS[] =  " STOP RUN IF THEN ELSE END-IF PERFORM CALL END-PERFORM END-CALL AUTHOR GO CONTINUE ";
...
int parseAlpha(char * ch){
    int count=0;
    while(isalpha(*ch) || *ch=='-' || *ch=='_' || isdigit(*ch)){
        ch = ch + 1;
        count++;
    }
    return count;
...
int parseDigitOrAlpha(char * ch,struct st_highlt * h){
    int count=0;
    int alpha=0;
    while(isdigit(*ch)){
        ch = ch + 1;
...
void isReserved(struct st_highlt * h){
    ...
    if(h->size<50){
        strcpy(test," ");
        strncpy(&test[1],h->token,h->size);
        test[h->size+1]='\0';
        toUpper(test);
        strcat(test," ");
        if(strstr(COBOL_RESERVED_WORDS, test)){
            h->color=color_blue;
        }
        if(strstr(COBOL_SPECIAL_WORDS, test)){
            h->color=color_pink;
        }        
    }
}
    ...
    while(lines!=NULL){
        ...
        do{
            ...
            if(*ch==' '){
                h->token=ch;
                type=TP_SPACES;
                h->size = parseSpaces(ch);
                h->color = color_black;
                ch+=h->size;
            }else if(*ch=='\''){
                h->token=ch;
                type=TP_STRING1;
                h->size = parseStringHigh(ch);
                ...
                h->token=ch;
                type=TP_ALPHA;
                h->size = parseAlpha(ch);
                h->color = color_white;
                isReserved(h);
                ch+=h->size;
            }else if(*ch=='*' && tp_before==TP_SPACES && pos==7){
                h->token=ch;
                type=TP_COMMENT;
                h->size = strlen(h->token);
                ...
                h->token=ch;
                type=TP_SYMBOL;
                h->size = parseSymbols(ch);
                h->color=color_green;
                if(h->size==2 && strncmp(h->token,"*>",2)==0){
                    h->color=color_dark_green;
                    type=TP_COMMENT;
                    h->size = strlen(h->token);
                    ch+=h->size;
                    break;
                }else if(tp_before==TP_SPACES && h->size==2 && strncmp(h->token,">>",2)==0){
                    h->color=color_dark_green;
                    type=TP_COMMENT;
                    h->size = strlen(h->token);
                    ch+=h->size;
        ...
        if(high->token!=NULL && high->size>0) high->token[strcspn(high->token,"\n")]=' ';
        if(high!=NULL){
            lines->high=high;
        }
